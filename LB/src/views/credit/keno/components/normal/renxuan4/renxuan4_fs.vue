    ...
    <li>
      <div class="renxuan-menu">
        <span :class="{ 'now-hov': renxuanMenuInfo.isSelectAll }">
          <a @click="selectAllNumber(calculateBetCounts, 8)"
             href="javascript:;">全</a>
        </span>
        <span :class="{ 'now-hov': renxuanMenuInfo.isSelectBig }">
          <a @click="selectBigNumber(calculateBetCounts, 8)"
             href="javascript:;">大</a>
        </span>
        <span :class="{ 'now-hov': renxuanMenuInfo.isSelectSmall }">
          <a @click="selectSmallNumber(calculateBetCounts, 8)"
             href="javascript:;">小</a>
        </span>
        <span :class="{ 'now-hov': renxuanMenuInfo.isSelectOdd }">
          <a @click="selectOddNumber(calculateBetCounts, 8)"
             href="javascript:;">奇</a>
        </span>
        <span :class="{ 'now-hov': renxuanMenuInfo.isSelectEven }">
          <a @click="selectEvenNumber(calculateBetCounts, 8)"
             href="javascript:;">偶</a>
        </span>
        <span :class="{ 'now-hov': renxuanMenuInfo.isSelectQuality }">
          <a @click="selectQualityNumber(calculateBetCounts, 8)"
             href="javascript:;">质</a>
        </span>
        <span :class="{ 'now-hov': renxuanMenuInfo.isSelectComposite }">
          <a  @click="selectCompositeNumber(calculateBetCounts, 8)" href="javascript:;">合</a>
        </span>
        <span>
...
export default {
  ...
  methods: {
    clickBall (ball) {
      ...
      if (flagCount >= 8 && !ball.isSelected) {
        this.$message({
          message: '最多只能选择8个数字',
          type: 'error'
        })
    ...
    calculateBetCounts () {
      ...
      this.numberList.map(item => {
        item.numberList.map(itemInner => {
          if (itemInner.isSelected) flagCount += 1
        })
      })
