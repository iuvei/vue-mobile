  ...
  methods: {
    ...
    calculateBetCounts () {
      ...
      if (twiceArr.length && onceArr.length) {
        let twiceInfoArr = []
        const twiceArrLength = twiceArr.length
        for (let i = 0; i <= twiceArrLength * twiceArrLength; i++) {
          const twiceNumber1 = Math.floor(i / twiceArrLength)
          const twiceNumber2 = i % twiceArrLength
          if (twiceNumber1 <= twiceNumber2 || twiceNumber1 === twiceArrLength) continue
          const countArr = []
          countArr.push(twiceArr[twiceNumber1].value)
          countArr.push(twiceArr[twiceNumber2].value)
          twiceInfoArr.push(countArr)
        }
        twiceInfoArr.map(item => {
          let flagArr = onceArr.map(item => item.value)
          item.map(itemInner => {
            flagArr = flagArr.filter(item => item !== itemInner)
          })
          count += flagArr.length
        })
      }
