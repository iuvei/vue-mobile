  ...
  methods: {
    ...
    calculateBetCounts () {
      ...
      this.numberList[0].numberList.map(item => {
        if (item.isSelected) thirdArr.push(item)
      })
      this.numberList[1].numberList.map(item => {
        if (item.isSelected) onceArr.push(item)
      })
      if (thirdArr.length && onceArr.length) {
        thirdArr.map(item => {
          const noRepeatArr = onceArr.filter(itemInner => itemInner.value !== item.value)
          noRepeatArr.map((item, index) => {
            count += index
          })
        })
      }
