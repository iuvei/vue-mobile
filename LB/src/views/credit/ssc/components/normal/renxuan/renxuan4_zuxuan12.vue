...
export default {
  ...
  methods: {
    ...
    calculateBetCounts () {
      let count = 0
      let trueCount = 0
      const twiceArr = this.numberList[0].numberList.filter(item => item.isSelected)
      const onceArr = this.numberList[1].numberList.filter(item => item.isSelected)
      twiceArr.map(item => {
        const everyCount = onceArr.filter(itemInner => itemInner.label !== item.label).length
        for (let i = 0; i < everyCount; i++) {
          count += i
        }
      })
      for (let key in this.checkboxType) {
        if (this.checkboxType[key]) trueCount++
      }
      if (trueCount < 4) trueCount = 0
      if (trueCount === 4) trueCount = 1
      this.selectedCounts = count * trueCount
