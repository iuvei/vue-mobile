      ...
      <div class="choice-number-box-container">
        <div v-for="item in childPlayList"
          ...
          <router-link :class="{ 'now-hov': itemInner.isSelected }"
                       ...
                       :key="itemInner.id"
                       :to="{ name: `ssc-im_${$route.matched[3].meta.sub_path}_${item['name_en']}_${itemInner['name_en']}` }"
        ...
        <div class="im-show-box">
          ...
          <div class="btns clear-fix">
            <a @click="choiceNumberReset"
               :class="{ 'disable': imGameIsOpening }"
               href="javascript:;">
              重新选号
              <img :src="rightIcon"
                   alt="rightIcon">
            ...
            <div class="bet-settings">
              <p>连续开奖</p>
              <el-radio-group v-model="lotteryCount">
                <el-radio v-for="(item, index) in betCountsData"
                          :key="index"
                          :label="item.value">{{item.label}}</el-radio>
              </el-radio-group>
              <div class="checkbox">
                <el-checkbox v-model="isStopWin">中奖后停止开奖</el-checkbox>
              </div>
            </div>
          </div>
        </div>
...
<script>
import layoutMixins from '../minxins-im'
import { lotteryImDefault } from '../../../config/game-default-config'

export default {
  mixins: [ layoutMixins ],
  data () {
    return {
      lotteryCount: lotteryImDefault.betCount,
      betCountsData: lotteryImDefault.betCountsData,
      isStopWin: lotteryImDefault.isStopWin
    }
  }
}
</script>
...
<style scoped>
  ...
  .choice-number-box-container > .im-show-box {
    position: absolute;
  ...
  .choice-number-box-container > .im-show-box .btns {
    text-align: center;
  }
  .choice-number-box-container > .im-show-box .btns .bet-settings {
    display: inline-block;
    margin-left: 30px;
  }
  .choice-number-box-container > .im-show-box .btns .bet-settings > p {
    text-align: left;
    font-size: 14px;
    color: #2c2c2c;
    margin-bottom: 13px;
  }
  .choice-number-box-container > .im-show-box .btns .bet-settings > .checkbox{
    text-align: left;
    margin-top: 14px;
  }
  ...
  .choice-number-box-container > .im-show-box .btns > a:nth-of-type(1) {
    width: 162px;
    position: relative;
    top: -20px;
    color: #7e6628;
    ...
    margin-right: 20px;
    border: 1px solid #a2966e;
    border-radius: 3px;
    box-shadow: 0 3px 4px 1px rgba(0, 0, 0, .3);
  }
  .choice-number-box-container > .im-show-box .btns > a:nth-of-type(2) {
    width: 162px;
    position: relative;
    top: -20px;
    color: #fff;
  ...
  .choice-number-box-container > .im-show-box .btns > a.disable {
    background-color: #777777;
    box-shadow: none;
    color: #fff;
    border: 1px solid rgb(181, 181, 181);
  }
  .choice-number-box-container > .im-show-box .btns > a.disable:hover {
    cursor: not-allowed;
  }
</style>
